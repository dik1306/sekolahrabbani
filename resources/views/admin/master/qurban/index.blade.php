...
@section('content')
    <div class="iq-navbar-header">
        ...
        <div class="title my-3">
            <h1 class="text-white" style="margin-left: 1rem">List Materi Qurban </h1>
        </div>
    </div>
    ...
    <div class="modal fade" id="add_materi" tabindex="-1" role="dialog" aria-labelledby="create_materi" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{route('materi-qurban.store')}}" enctype="multipart/form-data" method="post" >
                    @csrf
                ...
                <form action="#" method="post" enctype="multipart/form-data" id="editForm">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select name="jenjang_edit" id="jenjang_edit" class="form-control form-control-sm" required>
                                <option value="" disabled selected> </option>
                                <option value="1" >Kober - TK - SD </option>
                                <option value="2" >SMP </option>
                            </select>
                        ...
                        <div class="form-group" id="file_group">
                            <label for="link_video_edit" class="form-control-label">Link Video</label>
                            <input type="text" class="form-control" name="link_video_edit" id="link_video_edit">
                        </div>
        ...
        function edit_data_smp(id) {
            fetch('/master/qurban/' + id)
                ...
                .then(data => {
                    ...
                    $("#link_video_edit").val(data.link_video)
                    $("#editForm").attr('action', '/master/qurban/' + id)
                    $("input[name='_method']").val('PUT')
                })
        }

        function edit_data_tksd(id) {
            var url = "{{ route('master.update-materi-qurban', '') }}" + "/" + id;
            fetch('/master/qurban/' + id)
                .then(response => response.json())
                .then(data => {
                    $("#jenjang_edit").val(data.jenjang)
